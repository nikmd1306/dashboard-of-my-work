...
.claude/               — agent config (subagents, slash commands, hooks); read by both Claude Code and Cursor
...
13. **Never use IDE-specific planning modes.** Editors like Cursor, VS Code, etc. have built-in "plan mode" or similar features. Do not use them or suggest switching to them. All planning happens through execution plans in `docs/plans/` — that is the only planning system for this project.
14. **Write phases that are ready to execute, not ready to think about.** A phase is written for a different agent that will execute it without the planning context. Each phase must: focus on a single concern (don't combine data display with interactive forms, or API work with UI work); list the specific files to create or edit; resolve implementation choices (which components, which CSS classes, which props) so the executor writes code, not makes decisions. If describing a phase requires the word "or" for an implementation choice, pick one and log the decision. See `docs/plans/TEMPLATE.md` for phase-writing guidance.
15. **Run the `verifier` subagent after completing any code-producing task.** Whether it was a plan phase or an ad-hoc task, delegate to the `verifier` subagent (defined in `.claude/agents/verifier.md`) before considering work done. Pass it: (a) the list of files you created or modified, (b) a brief description of what was done, and (c) the plan file path and phase number if a plan was being executed. Do not report completion to the user until the verifier shows no blocking issues. When writing execution plans, include a verification step at the end of every phase (see `docs/plans/TEMPLATE.md`).
16. **Delegate context-heavy subtasks to subagents.** If a subtask is self-contained and likely to consume significant context — codebase exploration, documentation research, reading many files, running numerous searches, or other routine investigative work — delegate it to a general-purpose subagent instead of doing it inline. This preserves the main agent's context window for higher-level reasoning and decision-making. Give the subagent a precise description of what to find or do and what to return. If multiple independent subtasks can run in parallel, launch them concurrently.
